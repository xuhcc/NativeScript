...
export class Frame extends FrameBase {
    ...
    _onAttachedToWindow(): void {
        ...
        // if (this._currentEntry && this["_hasParentManager"] && !application.android.paused && manager && !manager.findFragmentByTag(this._currentEntry.fragmentTag)) {
    ...
    protected _processNextNavigationEntry(): void {
        ...
        // if (fromAttachedToWindow && entry && manager && this["_hasParentManager"] && !manager.findFragmentByTag(entry.fragmentTag)) {
        //     return;
        // }
        if (entry && manager && !manager.findFragmentByTag(entry.fragmentTag)) {
            ...
            this._cachedTransitionState = getTransitionState(this._currentEntry);
    ...
    public _navigateCore(newEntry: BackstackEntry) {
        ...
        const isNestedDefaultTransition = this["_hasParentManager"] && !currentEntry;
...
function getTransitionState(entry: BackstackEntry): TransitionState {
    ...
    // transitionState.enterTransition = { transition: expandedEntry.enterTransitionListener.transition.clone(), entry: entry };
    // transitionState.exitTransition = { transition: expandedEntry.exitTransitionListener.transition.clone(), entry: entry };
    // transitionState.reenterTransition = { transition: expandedEntry.reenterTransitionListener.transition.clone(), entry: entry };
    // transitionState.returnTransition = {
    //     transition: expandedEntry.returnTransitionListener.transition.clone(), entry: entry
    // };
...
function restoreTransitionState(entry: BackstackEntry, snapshot: TransitionState): void {
    ...
    // if (snapshot.enterTransition) {
    //     expandedEntry.enterTransitionListener = addNativeTransitionListener(snapshot.enterTransition.entry, snapshot.enterTransition.transition);
    // }

    // if (snapshot.exitTransition) {
    //     expandedEntry.exitTransitionListener = addNativeTransitionListener(snapshot.exitTransition.entry, snapshot.exitTransition.transition);
    // }

    // if (snapshot.reenterTransition) {
    //     expandedEntry.reenterTransitionListener = addNativeTransitionListener(snapshot.reenterTransition.entry, snapshot.reenterTransition.transition);
    // }

    // if (snapshot.returnTransition) {
    //     expandedEntry.returnTransitionListener = addNativeTransitionListener(snapshot.returnTransition.entry, snapshot.returnTransition.transition);
    // }
