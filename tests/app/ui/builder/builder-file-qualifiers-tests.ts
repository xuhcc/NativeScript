...
function loadPageXmlWithComponent(prefix: string) {
    return `
        <Page xmlns:cc="${prefix}/component">
            <cc:MyComponent/>
        </Page>`;
}
...
export function testCustomComponentWithXmlCssJs_NoQualifiers() {
    // Arrange
    registerTestModule(`${testPrefix}/test.xml`, () => loadPageXmlWithComponent(testPrefix));
...
// TODO test file qualifiers:
// 0. straight case
// 1. page css, xml, js, code-only, xml-only, code-file, css-file
// 2. custom component css, xml, js, code-only, xml-only, code-file, css-file
